...
public class HttpUtil {
	...
	public static String post(String url, String json) {
		return post(url, null, json, DEFAULT_CHARSET);
	}

	public static String post(String url, Map<String, String> headerMap, String json, String charset) {
		logger.info("执行http post请求,url[{}],参数[{}],编码格式[{}]", new Object[] { url, json });
		String result = "";
		...
		try {
			...
			HttpPost httpost = new HttpPost(url);
			if(headerMap!=null){				
				for(Map.Entry<String, String> keySet:headerMap.entrySet()){				
					httpost.setHeader(keySet.getKey(), keySet.getValue());
				}
			}
		...
		try {
			...
			if (paramsMap != null) {
				if ("json".equals(contentType)) {
					...
					httpPut.setEntity(new StringEntity(jsonParams, charset));
				} else {
